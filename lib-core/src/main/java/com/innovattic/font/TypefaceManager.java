...
public class TypefaceManager {
	...
	static {
		// Initialize the style map. The style map is used to determine which font style must be
		// used if the requested style is not available.
	...
	/**
	 * Initializes the singleton instance of the TypefaceManager. It will read the given xml font
	 * file pointed to by xmlResource, but doesn't yet load the fonts.
	 */
	public static synchronized void initialize(Context context, int xmlResource) {
		...
		if (INSTANCE.mXmlResource != xmlResource) {
			Log.w(TAG, "Singleton instance of TypefaceManager was initialized with a different " +
					"xml font file previously. Re-initialization will not occur.");
		}
		if (!INSTANCE.mContext.equals(context)) {
			Log.w(TAG, "Singleton instance of TypefaceManager was initialized with a different " +
					"context previously. Re-initialization will not occur.");
		}
	}

	/**
	 * Returns the singleton instance of the TypefaceManager. It will throw an exception if it is
	 * called before {@link #initialize(Context, int) initialization}.
	 *
	 * @return The TypefaceManager
	 */
	public static TypefaceManager getInstance() {
		if (INSTANCE == null) {
			throw new IllegalStateException("Cannot use TypefaceManager.getInstance() before it " +
					"is initialized. Use TypefaceManager.initialize(Context, int) to initialize " +
					"the TypefaceManager.");
		}
		return INSTANCE;
	}
	 ...
	 * @param xmlResource The resource id of an xml file that specifies the fonts that are present
	 *        in the assets.
	 * @return The number of unique font files in the xml file that exist. The font files are not
	 *         yet opened or verified, it only means that they exist.
	 */
	...
	/**
	 * Returns the typeface identified by the given font namen in the normal style. The font must be
	 * a name defined in any nameset in the font xml file. Similar to {@link
	 * #getTypeface(String, int) getTypeface(name, Typeface.NORMAL)}.
	 *
	...
	/**
	 * Convenience method to set the typeface of the target view to the font identified by fontName.
	 * The text style that will be used is {@code Typeface#NORMAL}. Returns false if the font wasn't
	 * found or couldn't be loaded, returns true otherwise.
	 *
	 * @param target A TextView in which the font must be used.
	 * @param fontName The name of the font. Must match a name defined in a name tag in the xml font
	 *        file.
	 * @return {@code true} if the font could was set in the target, {@code false} otherwise.
	 */
	...
	/**
	 * Convenience method to set the typeface of the target view to the font identified by fontName
	 * using the textStyle. The textStyle must be one of the constants defined by {@code Typeface}.
	 * Returns false if the font wasn't found or couldn't be loaded, returns true otherwise.
	 *
	 * @param target A TextView in which the font must be used.
	 * @param fontName The name of the font. Must match a name defined in a name tag in the xml font
	 *        file.
	...
	/**
	 * Parses the xml font file. After this method, {@link #mFonts} will contain all fonts
	 * encountered in the xml font file for which at least one of the defined font file(s) exist.
	 */
	private void parse() {
						...
						if (tag.equals(TAG_FAMILY)) {
							...
							if (!font.styles.isEmpty()) {
								for (String name : font.names) {
									// Don't override duplicate font names
					...
					case XmlPullParser.TEXT:
						...
						if (isName) {
							...
							if (Arrays.binarySearch(fontAssets, ttf) < 0) {
								Log.w(TAG, "Couldn't find font in the assets: " + ttf);
							} else {
								// Add the style
	...
	/**
	 * Returns the style that comes after the given style in a fileset in the font xml file. This
	 * order is defined by Android in the definition of the system fonts and vendor fonts (see
	 * https://github.com/android/platform_frameworks_base/blob/master/data/fonts/vendor_fonts.xml).
	 *
	 * @param style An internal style
	 * @return The internal style after the given one
	 */
	...
	/**
	 * Adds style mappings for all styles that are not loaded in the given font. A font may be
	 * defined without all four styles regular, bold, italic and bold-italic. In that case, the
	 * missing styles will be mapped to the most preferred style that is present.
	 *
	 * @param font A font
	 */
	...
	private String[] getAvailableFontFiles() {
		...
		try {
			...
			final String[] fontAssets = fontAssetList.toArray(new String[fontAssetList.size()]);
	...
	/**
	 * Returns if all set bits in the needle are not set in the haystack. In other words, if the
	 * haystack is missing all the bits from the needle, this returns {@code true}.
	 *
	 * @param needle All bits set to 1 in this byte will be checked.
	 * @param haystack The bits in this byte will be checked.
	 * @return {@code true} iff all bits that need to be checked in the haystack are 0.
	 */
	 ...
	 * in the corresponding view instance, we can apply the properties at any time with a shared
	 * static method.
	 *
